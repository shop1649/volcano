...
>
> **측정 시점**: 이 문서의 수치는 커밋 c8d90e7 시점 분석기로 잰 것이다. 그 뒤 적대적 검토에서 분석기를 고쳤다
> (27fb18b: 공용 그림자 추정기 — 레퍼런스 그림자가 항상 0이던 결함, 대사 `lead_s` = 말소리 시작 − 자막 시작으로 부호 통일;
> 3be36ea·fcb2bfd: 컷 구조·정보 공개 순서·효과음 partial 상태 등). 이 표를 바뀐 분석기로 다시 재지는 않았다.
> 따라서 `shadow_px`·`lead_s` 행은 지금 코드의 출력과 다를 수 있다. 바뀐 부분은 해당 단위 테스트로 확인했다.
