...
| 자동 테스트 | 저장소에서 `pytest -q`(느린 테스트 포함) | **993개 모두 통과**(44분 51초, 커밋 d9b6b63 이후 코드) | 다른 OS·Python 버전 |
...
### 2차 (고친 번들, 커밋 c9fa60e, 파일 364개)

- 복원 블록을 그대로 실행했다. 파일 364개가 저장소와 바이트 단위로 같다.
- `setup.sh` 종료 0. `doctor` 필수 미충족 0. 테스트 자산 5단계 모두 종료 0.
- `pytest -m "not slow"`: 827 passed / 2 failed / 2 skipped.
  - 실패 2건은 번들에 넣지 않는 예제 MP4 를 요구한 테스트다. b214659 에서 건너뛰게 고쳤다.
  - 같은 폴더에서 다시 돌린 결과: 22 passed, 4 skipped.
- test-restore-001 을 번들의 plan 그대로 다시 렌더했다.
  - **MP4 가 1차 결과와 sha256 까지 같다**(`6a1cd845…`).
  - melt 대조 통과: SSIM 0.9942 / 소리 0.9998.
  - QA 164 / 0 / 85. 관문은 사람 확인 12건 때문에 불합격이다.
- 기록: `docs/validation/final_restore_log.md` 2차 절.
