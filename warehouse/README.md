...
| `original_author`, `original_author_basis`, `original_url`, `reposter`, `credits[]` | 메타데이터 + 설명의 출처 표기('출처', 'credit', 'via', '@')로 원작자와 재업로더를 나눔. 근거(`basis`)를 함께 기록. 크레딧이 없으면 업로더를 원작자로 두되 "원작자 여부 미확인"으로 표시 |
...
| `reviews[]` | 영상을 **직접 본** 사람/에이전트의 기록: `watched_by`, `watched_at`, `intensity`, `reversal`, `format_fit`(1-5), `notes`, `watermark` |
| `scores{}` | `recency`, `intensity`, `reversal`, `quality`, `format_fit`, `total`, `recency_label`, `age_days`, `unmeasured[]` |
...
3. **순위는 플랫폼별, 확인된 조회수로만**: 조회수를 모르는 후보는 순위 없이 뒤에 `views_unknown` 표시로 나열.
4. **오래된 바이럴은 최근이 아님**: 최근성은 확인 시각 기준 게시일 경과일로만 판정(기본 30일, `queries.yaml` 의 `selection.recent_days`). 조회수나 우리가 처음 본 날짜는 영향을 주지 않음. 게시일을 모르면 "게시일 모름"(최근 아님). **재업로드**로 보이면(원작자 ≠ 업로더, 또는 원본 URL 이 따로 있음) 재업로드 날짜가 아니라 원본 게시일로 판정하며, 원본 게시일을 모르면 "재업로드: 원본 게시일 모름"(최근 아님). 원본 게시일은 검토 때 `--original-published-at` 으로 입력.
...
8. **차단은 차단으로 기록**: 막힌 플랫폼은 `blocked`, 로그인이 필요하면 `login_required`, 그 외는 `error` 와 오류 원문을 기록하고 다음 플랫폼/후보로 넘어감. 성공한 척하지 않음. 우회(미러·프록시·Invidious 등)하지 않음.
...
| TikTok | 키워드 → 해시태그 `tiktok.com/tag/<tag>`, `@user`, 영상 URL | `view_count`/`play_count` (재생수) | 설치된 yt-dlp(2026.08.19)가 `tiktok:tag` 추출기를 작동 안 함으로 표시 → 검색 기록에 남김. 날짜 필터 없음(최근: 게시일로 거름) |
| Instagram | 게시물/릴스 URL. 해시태그·계정은 로그인 필요 | 메타데이터에 있을 때만 | `local.yaml` 의 `sourcing.cookies.instagram` 이 없으면 요청하지 않고 `login_required` |
...
python -m shortkit source add-url URL [--file 직접받은.mp4]  # 수동 수집
