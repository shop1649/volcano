...
Audio: mixed in numpy (sample exact, deterministic): BGM (tempo pre-step + section trim, gain,
  ...
  kept originals, SFX at t; integrated LUFS -> gain to target; true-peak safety limiter.
  build/mix.wav and build/stems/{bgm,originals,sfx}.wav are written for QA.
"""
...
BLUR_SIGMA_FRAC = 1 / 6.0   # blur ops: gaussian sigma = max(w, h) * frac (SOURCE px)
EPS_T = 1e-3
...
def _oversampled_peak(x: np.ndarray) -> np.ndarray:
    ...
    pk = np.abs(up).max(axis=1)[: 4 * n]
    pk = np.pad(pk, (0, 4 * n - len(pk)))
    return np.maximum(pk.reshape(n, 4).max(axis=1), np.abs(x).max(axis=1))


def _smooth_min_gain(g: np.ndarray, blk: int = 240, W: int = 4) -> np.ndarray:
    """Look-ahead block minimum + widened min filter + moving average; never exceeds ``g``."""
...
def foreground_limiter_gain(bg: np.ndarray, fg: np.ndarray, ceiling_db: float) -> np.ndarray:
    """Stem-aware safety limiter: gain (<=1) applied ONLY to the foreground stems (kept originals +
    SFX) so that |bg| + |fg*k| stays under the true-peak ceiling.  The BGM is never modulated by
    it -- SFX (or cuts) must never duck the BGM (user rule)."""
    c = db2lin(ceiling_db)
    room = np.maximum(c - _oversampled_peak(bg), 0.0)
    fpk = _oversampled_peak(fg)
    need = np.where(fpk > room, room / np.maximum(fpk, 1e-12), 1.0)
    return _smooth_min_gain(np.minimum(1.0, need))
...
def mix_audio(r: ResolvedEdit, build: Path) -> dict:
    ...
    rep: dict = {"sample_rate": sr, "samples": n, "warnings": []}
    ...
    try:
        ...
        for s in a.sfx:
            if not s.path:
                rep["warnings"].append(f"효과음 {s.id}({s.type}) 파일 미해결 → 테스트 모드라서 넣지 않음")
                continue
            x = read_audio(paths.absp(s.path), sr=sr, mono=False) * db2lin(s.gain_db)
            s0 = int(round(s.t * sr))
            e0 = min(n, s0 + len(x))
            if e0 > s0:
                sfx[s0:e0] += x[: e0 - s0]
        mix = bgm + orig + sfx
        pre = _measure(mix, sr, tmpdir)
        rep["pre_norm"] = pre
        ...
        if pre.get("integrated_lufs") is None or pre["integrated_lufs"] < -69.0:
            ...
            gain = db2lin(target - pre["integrated_lufs"])
            bg_tp = _true_peak_db(bgm)
            for _ in range(4):
                # the BGM alone must leave >= 1 dB headroom; otherwise lower the static gain
                if bg_tp + 20 * math.log10(gain) > ceiling - 1.0:
                    gain = db2lin(ceiling - 1.0 - bg_tp)
                    rep["bgm_headroom_capped"] = True
                k = foreground_limiter_gain(bgm * gain, fg * gain, ceiling)
                post = _measure(gain * (bgm + fg * k[:, None]), sr, tmpdir)
                err = target - (post.get("integrated_lufs") or target)
                if abs(err) <= LOUDNESS_ITER_TOL_LU or rep.get("bgm_headroom_capped"):
                    break
                gain *= db2lin(err)
        rep["norm_gain_db"] = round(20 * math.log10(gain), 3)
        rep["fg_limiter_max_reduction_db"] = round(float(-20 * np.log10(max(1e-9, float(k.min())))), 3)
        ...
        for s in a.sfx:
            if not s.path:
                continue
            s0 = int(round(s.t * sr))
            seg = k[s0:min(n, s0 + int(0.5 * sr))]
            if len(seg):
                red = round(float(-20 * np.log10(max(1e-9, float(seg.min())))), 2)
                sfx_lim.append({"id": s.id, "type": s.type, "t": s.t, "reduction_db": red})
                if red > 3.0:
                    rep["warnings"].append(f"효과음 {s.id}({s.type}) 이 안전 리미터로 {red} dB 줄어듦 → 효과음/BGM 레벨 균형 조정 필요")
        rep["sfx_limiting"] = sfx_lim
        kk = k[:, None]
        bgm, orig, sfx = bgm * gain, orig * gain * kk, sfx * gain * kk
        mix = bgm + orig + sfx
        ...
        if got is not None and abs(got - target) > 1.0:
            rep["warnings"].append(f"목표 음량 {target} LUFS 에 못 맞춤(측정 {got} LUFS): 믹스 균형/헤드룸 확인 필요")
    finally:
...
class ClipReader:
    ...
    def _apply_blur(self, fr: np.ndarray, pts: float) -> np.ndarray:
        ...
        for bl in self.clip.blur:
            ...
            if x1 > x0 and y1 > y0:
                sig = max(1.0, max(bl.w * self.kx, bl.h * self.ky) * BLUR_SIGMA_FRAC)
