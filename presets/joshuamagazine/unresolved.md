...
자동 생성: `shortkit preset unresolved` (2026-09-24T17:13:38+00:00). 손으로 고치지 말 것 — 원본은 settings_registry.yaml 과 각 산출물.
...
|---|---|---|---|---|
| 최신 100편 목록·게시일 고정 | 못 잼 | 포맷 분류·모든 측정의 기준 표본이 없음 → 모든 스타일 값이 임시값 | blocked_network | reference/latest100.json status=없음 videos=0 blocker=None |
| 조회수 80만 이상 영상 전체 분석 | 못 잼 | 고조회 영상의 공통 구조를 확인할 수 없음 | blocked_network | reference/high_views.json status=없음 videos=0 |
...
| 글꼴 식별(IoU 상한 + 후보 검증) | 못 잼 | 글꼴이 레퍼런스와 같은지 판정 불가(현재 임시 글꼴) | blocked_network | fonts_report.json status=없음 |
...
상태별 개수: fixed_by_rule=13, not_applicable=23, unmeasured=251
...
| 화살표·원 등 장식 스타일 불일치 | 11 | `decorations.arrow.blink_hz`, `decorations.arrow.color`, `decorations.arrow.outline_color`, `decorations.arrow.outline_px`, `decorations.arrow.size_px`, `decorations.box.blink_hz`, `decorations.box.color`, `decorations.box.stroke_px`, `decorations.circle.blink_hz`, `decorations.circle.color`, `decorations.circle.stroke_px` |
| 확대·정지·전환의 크기/길이 불일치 | 9 | `motion.freeze.hold_s`, `motion.speed.slowmo_factor`, `motion.transitions.crossfade.dur_s`, `motion.transitions.default`, `motion.transitions.flash.color`, `motion.transitions.flash.dur_s`, `motion.zoom.dur_s`, `motion.zoom.ease`, `motion.zoom.scale_to` |
