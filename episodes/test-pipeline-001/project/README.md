...
- 생성 2026-09-24T21:49:49+00:00 · 프리셋 `joshuamagazine-v1` · 포맷 `UNCLASSIFIED` · 모드 `test` (테스트: 파이프라인 검증용, 게시용 아님)
...
- 전체 음량: 타임라인 volume 필터 -0.24 dB (출처: episodes/test-pipeline-001/build/render_report.json)
- 효과음/원본 소리 안전 리미터: 마스터가 최대 3.0 dB 줄인 것을 `episodes/test-pipeline-001/build/fg_gain.json` 에서 프레임별로 읽어 각 클립 volume 키프레임으로 재현 (마스터 리미터는 샘플 단위, MLT 키프레임은 프레임(1/fps) 단위라 피크 순간은 약간 다를 수 있음)
...
- 전체 음량 이득 -0.24 dB 를 각 오디오 클립 음량에 더해 넣음(마스터 버스 없음)
...
| id | 역할 | 시간(s) | 텍스트 |
|---|---|---|---|
| c_title | 제목 | 0.00–19.25 | 움직임 테스트 영상 |
...
- MLT 프로젝트를 melt 로 실제 렌더해 마스터 MP4 와 같은 시각 1초 격자로 비교: **통과**
  ...
  - 소리 RMS 포락선 상관 0.9965 (기준 ≥ 0.9), 초별 레벨 차 최대 0.76 dB
...
|---|---|---|---|---|---|
| 0 | 0.9981 | 0.251 | 0.16 | 통과 | 통과 |
| 1 | 0.9981 | 0.273 | 0.03 | 통과 | 통과 |
| 2 | 0.9982 | 0.27 | 0.01 | 통과 | 통과 |
| 3 | 0.9981 | 0.268 | 0.03 | 통과 | 통과 |
| 4 | 0.9981 | 0.26 | 0.04 | 통과 | 통과 |
| 5 | 0.9981 | 0.262 | 0.02 | 통과 | 통과 |
| 6 | 0.998 | 0.288 | -0.0 | 통과 | 통과 |
