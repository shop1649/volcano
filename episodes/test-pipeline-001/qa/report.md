...
- 레퍼런스: 없음(못 잼)
- 측정 시각: 2026-09-24T22:24:17+00:00 / 도구: OCR=5.3.4, 얼굴검출=shortkit.clean.faces haar frontal+profile (shortkit.HaarCascade(numpy))

## 최종 관문: **불합격**
- ✗ G2: 필수 항목 못 잼 3건 — 못 잼은 완료가 아님 — caption.font:c_desc, caption.font:c_dlg, caption.font:c_sit4
- △ P1: 프리셋 미측정(임시값) 키 255개
- △ P2: 레퍼런스 대비 못 잼 53건
- △ P3: 테스트 모드 출력(파이프라인 검증용) — 게시 불가

## 요약
- 전체 213행: 같다 152 / 다르다 0 (의도한 변경 0, 의도하지 않음 0) / 못 잼 61
- 계획 대비(출력이 계획대로인가): {'same': 152, 'unmeasured': 8}
...
| 항목 | 레퍼런스 | 기대 | 출력 측정 | 판정(같다/다르다/못 잼) | 의도한 변경 | 근거 |
|---|---|---|---|---|---|---|
| 자막 글꼴 [c_title·title] "움직임 테스트 영상" | 못 잼 | Noto Sans CJK KR Black | {"verdict":"identical","verdict_ko":"동일","iou_expected":0.9883,"margin":0.0727,"top":"Not… | 같다 | 아니오 | t=0.12s [프레임](frames/cap_c_title_0000120.png) 판정: 동일(identical) — IoU 0.988 >= 천장 p10 0.980; 차순위 대비 차이 +0.073 > 잡음 … |
| 자막 글꼴 [c_desc·description] "파이프라인 검증용 합성 샘플" | 못 잼 | Noto Sans CJK KR Bold | {"verdict":"similar","verdict_ko":"유사(동일 확정 불가)","iou_expected":0.9511,"margin":0.0573,"t… | 못 잼 | 아니오 | t=0.32s [프레임](frames/cap_c_desc_0000320.png) 판정: 유사(similar, 동일 확정 불가) — 같다고 쓰지 않음. IoU 0.951 < 천장 p10 0.959; 차순위 … |
| 자막 글꼴 [c_spk·speaker] "창가 남성" | 못 잼 | Noto Sans CJK KR Bold | {"verdict":"identical","verdict_ko":"동일","iou_expected":0.9587,"margin":0.0532,"top":"Not… | 같다 | 아니오 | t=0.57s [프레임](frames/cap_c_spk_0000570.png) 판정: 동일(identical) — IoU 0.959 >= 천장 p10 0.951; 차순위 대비 차이 +0.053 > 잡음 … |
| 자막 글꼴 [c_sit1·situation] "창가 남성이 일어선다" | 못 잼 | Noto Sans CJK KR Black | {"verdict":"identical","verdict_ko":"동일","iou_expected":0.9729,"margin":0.0531,"top":"Not… | 같다 | 아니오 | t=1.54s [프레임](frames/cap_c_sit1_0001540.png) 판정: 동일(identical) — IoU 0.973 >= 천장 p10 0.967; 차순위 대비 차이 +0.053 > 잡음 … |
| 자막 글꼴 [c_sit2·situation] "다시 자리에 앉는다" | 못 잼 | Noto Sans CJK KR Black | {"verdict":"identical","verdict_ko":"동일","iou_expected":0.9701,"margin":0.0621,"top":"Not… | 같다 | 아니오 | t=4.34s [프레임](frames/cap_c_sit2_0004340.png) 판정: 동일(identical) — IoU 0.970 >= 천장 p10 0.968; 차순위 대비 차이 +0.062 > 잡음 … |
| 자막 글꼴 [c_sit3·situation] "뒷줄 남성이 손을 든다" | 못 잼 | Noto Sans CJK KR Black | {"verdict":"identical","verdict_ko":"동일","iou_expected":0.9634,"margin":0.0513,"top":"Not… | 같다 | 아니오 | t=7.94s [프레임](frames/cap_c_sit3_0007940.png) 판정: 동일(identical) — IoU 0.963 >= 천장 p10 0.958; 차순위 대비 차이 +0.051 > 잡음 … |
| 자막 글꼴 [c_dlg·dialogue] ""저기 봐, 들어온다!"" | 못 잼 | Noto Sans CJK KR Black | {"verdict":"similar","verdict_ko":"유사(동일 확정 불가)","iou_expected":0.9622,"margin":0.1321,"t… | 못 잼 | 아니오 | t=10.22s [프레임](frames/cap_c_dlg_0010219.png) 판정: 유사(similar, 동일 확정 불가) — 같다고 쓰지 않음. IoU 0.962 < 천장 p10 0.964; 차순위 … |
| 자막 글꼴 [c_rx·reaction] "갸우뚱?" | 못 잼 | Noto Sans CJK KR Black | {"verdict":"identical","verdict_ko":"동일","iou_expected":0.9802,"margin":0.0842,"top":"Not… | 같다 | 아니오 | t=15.32s [프레임](frames/cap_c_rx_0015319.png) 판정: 동일(identical) — IoU 0.980 >= 천장 p10 0.970; 차순위 대비 차이 +0.084 > 잡음 … |
| 자막 글꼴 [c_sit4·situation] "두 사람이 고개를 기울인다" | 못 잼 | Noto Sans CJK KR Black | {"verdict":"similar","verdict_ko":"유사(동일 확정 불가)","iou_expected":0.9686,"margin":0.0532,"t… | 못 잼 | 아니오 | t=15.34s [프레임](frames/cap_c_sit4_0015339.png) 판정: 유사(similar, 동일 확정 불가) — 같다고 쓰지 않음. IoU 0.969 < 천장 p10 0.972; 차순위 … |
...
| 자막 글꼴 [dialogue] (레퍼런스 대비) | 못 잼 | {"text.roles.dialogue.font_name":"Noto Sans CJK KR Black","text.roles.dialogue.bold":true} | — | 못 잼 (참고) | 아니오 | 레퍼런스 미측정(임시값) 키 2개: text.roles.dialogue.font_name, text.roles.dialogu… |
...
| 자막 표시 시간 [title] (레퍼런스 대비) | 못 잼 | {"text.roles.title.timing.lead_s":0.0,"text.roles.title.timing.min_dur_s":1.0,"text.roles… | {"min_dur_s":19.267} | 못 잼 (참고) | 아니오 | 레퍼런스 미측정(임시값) 키 3개: text.roles.title.timing.lead_s, text.roles.title.… |
...
- **필수** 자막 글꼴 [c_desc·description] "파이프라인 검증용 합성 샘플" (`caption.font:c_desc`): 판정: 유사(similar, 동일 확정 불가) — 같다고 쓰지 않음. IoU 0.951 < 천장 p10 0.959; 차순위 대비 차이 +0.057 > 잡음 0.038; 글자별 검사 통과 — 제작 영향: 글꼴이 맞는지 확인하지 못함 → 글자 인상이 다를 수 있음
- **필수** 자막 글꼴 [c_dlg·dialogue] ""저기 봐, 들어온다!"" (`caption.font:c_dlg`): 판정: 유사(similar, 동일 확정 불가) — 같다고 쓰지 않음. IoU 0.962 < 천장 p10 0.964; 차순위 대비 차이 +0.132 > 잡음 0.017; 글자별 검사 통과 — 제작 영향: 글꼴이 맞는지 확인하지 못함 → 글자 인상이 다를 수 있음
- **필수** 자막 글꼴 [c_sit4·situation] "두 사람이 고개를 기울인다" (`caption.font:c_sit4`): 판정: 유사(similar, 동일 확정 불가) — 같다고 쓰지 않음. IoU 0.969 < 천장 p10 0.972; 차순위 대비 차이 +0.053 > 잡음 0.007; 글자별 검사 통과 — 제작 영향: 글꼴이 맞는지 확인하지 못함 → 글자 인상이 다를 수 있음
...
## 결함 기록 (defects.jsonl)
- 열림 3 / 이번에 해결 확인 3 / 재발 1 / 새로 등록 0
- 같은 검사 재확인: test-coverage-001: audio.sfx.count=ok, audio.sfx.no_event=ok, audio.sfx.placement=ok, caption.font=problem
- 같은 검사 재확인: test-qa-bad: audio.sfx.count=problem, audio.sfx.no_event=problem, audio.sfx.placement=ok, caption.font=problem
- 같은 검사 재확인: test-qa-good: audio.sfx.count=ok, audio.sfx.no_event=ok, audio.sfx.placement=ok, caption.font=ok
