...
- 측정 대상: `episodes/test-pipeline-001/output/test-pipeline-001.mp4` (sha256 `459c922f3926091a…`, 1080x1920, 21.27s)
- 원칙: 최종 MP4 만 측정(타임라인/코드가 맞다고 출력이 맞다고 보지 않음)
- 프리셋: joshuamagazine-v1 / 포맷 UNCLASSIFIED / 모드 **test**
- 레퍼런스: 없음(못 잼)
- 측정 시각: 2026-09-24T18:52:57+00:00 / 도구: OCR=5.3.4, 얼굴검출=얼굴 검출기 없음: 이 OpenCV 빌드(cv2 5.0.0)에는 Haar cascade(CascadeClassifier/cv2.data)가 없고, 얼굴 모델 파일(assets/models/face_detection_yunet*.onnx 또는 $SHORTKIT_FACE_MODEL)도 없음

## 최종 관문: **불합격**
- ✗ G1: 의도하지 않은 차이(다르다) 8건 — caption.font:c_desc, caption.font:c_spk, audio.sfx.placement:fx1:gain, audio.sfx.placement:fx2:gain, audio.sfx.placement:fx3:gain, audio.sfx.placement:fx4:gain, audio.sfx.placement:fx5:gain, cover.frame:frame
- △ P1: 프리셋 미측정(임시값) 키 251개
- △ P2: 레퍼런스 대비 못 잼 53건
- △ P3: 테스트 모드 출력(파이프라인 검증용) — 게시 불가

## 요약
- 전체 209행: 같다 144 / 다르다 8 (의도한 변경 0, 의도하지 않음 8) / 못 잼 57
- 계획 대비(출력이 계획대로인가): {'same': 144, 'different': 8, 'unmeasured': 4}
...
| 자막 위치 [c_spk·speaker] "창가 남성" | — | {"center":[739.9,772.9],"bbox":[661.9,753.4,156.0,39.0],"resolution":[1080,1920]} | {"center":[737.5,772.0],"bbox":[644,751,187,42],"resolution":[1080,1920],"dx":-2.4,"dy":-… | 같다 | 아니오 | t=0.57s [프레임](frames/cap_c_spk_0000570.png) 에피소드 위치 지정(pos) |
| 자막 크기·줄 수 [c_spk·speaker] "창가 남성" | 못 잼 | {"fill_h":39.0,"lines":1,"max_width_px":420.0,"max_lines":1,"size_px":40.0} | {"fill_h":42,"fill_w":187,"lines":1,"size_px_est":43.1} | 같다 | 아니오 | t=0.57s [프레임](frames/cap_c_spk_0000570.png) 채움색 화소 높이 vs (기대 bbox 높이 − 2×외곽선) |
...
| 자막 문구(OCR) [c_desc·description] "파이프라인 검증용 합성 샘플" | — | 파이프라인 검증용 합성 샘플 | {"ocr":"파 이 프 라인 검 증 용 합성 샘플","similarity":1.0,"match":"ocr"} | 같다 | 아니오 | t=0.32s [프레임](frames/cap_c_desc_0000320.png) 굵은 글꼴 OCR 오차가 있어 유사도로 판정 |
...
| 자막 색·외곽선·박스 [c_spk·speaker] "창가 남성" | 못 잼 | {"color":"#FFFFFF","outline_px":0.0,"outline_color":"#000000","box":true,"box_alpha":0.65… | {"fill_color":"#FFFFFF","outline_px":0,"outline_color":"#52504E","box_alpha":0.622,"highl… | 같다 | 아니오 | t=0.57s [프레임](frames/cap_c_spk_0000570.png) |
...
| 항목 | 레퍼런스 | 기대 | 출력 측정 | 판정(같다/다르다/못 잼) | 의도한 변경 | 근거 |
|---|---|---|---|---|---|---|
| 자막 글꼴 [c_title·title] "움직임 테스트 영상" | 못 잼 | Noto Sans CJK KR Black | {"best":"Noto Sans CJK KR Black","iou_expected":0.9883,"scores":[{"font":"Noto Sans CJK K… | 같다 | 아니오 | t=0.12s [프레임](frames/cap_c_title_0000120.png) |
| 자막 글꼴 [c_desc·description] "파이프라인 검증용 합성 샘플" | 못 잼 | Noto Sans CJK KR Bold | {"best":"Noto Sans CJK KR Bold","iou_expected":0.4188,"scores":[{"font":"Noto Sans CJK KR… | 다르다 | 아니오 | t=0.32s [프레임](frames/cap_c_desc_0000320.png) 기대 글꼴 IoU 0.4188 ≪ 같은 글꼴 천장 0.951 — 다른(대체) 글꼴로 그려진 것으로 보임, 후보 글꼴 중에도 … |
| 자막 글꼴 [c_spk·speaker] "창가 남성" | 못 잼 | Noto Sans CJK KR Bold | {"best":"NanumGothic","iou_expected":0.3756,"scores":[{"font":"Noto Sans CJK KR Bold","io… | 다르다 | 아니오 | t=0.57s [프레임](frames/cap_c_spk_0000570.png) 기대 글꼴 IoU 0.3756 ≪ 같은 글꼴 천장 0.951 — 다른(대체) 글꼴로 그려진 것으로 보임, 후보 글꼴 중에도 … |
...
| 자막 글꼴 [c_rx·reaction] "갸우뚱?" | 못 잼 | Noto Sans CJK KR Black | {"best":"Noto Sans CJK KR Black","iou_expected":0.9802,"scores":[{"font":"Noto Sans CJK K… | 같다 | 아니오 | t=15.32s [프레임](frames/cap_c_rx_0015319.png) |
| 자막 글꼴 [c_sit4·situation] "두 사람이 고개를 기울인다" | 못 잼 | Noto Sans CJK KR Black | {"best":"Noto Sans CJK KR Black","iou_expected":0.8462,"scores":[{"font":"Noto Sans CJK K… | 같다 | 아니오 | t=15.34s [프레임](frames/cap_c_sit4_0015339.png) |
...
| 자막 글꼴 [speaker] (레퍼런스 대비) | 못 잼 | {"text.roles.speaker.font_name":"Noto Sans CJK KR Bold","text.roles.speaker.bold":true} | {"best":"NanumGothic"} | 못 잼 (참고) | 아니오 | 레퍼런스 미측정(임시값) 키 2개: text.roles.speaker.font_name, text.roles.speaker.… |
...
| 자막 등장 모션 [c_spk·speaker] "창가 남성" | 못 잼 | {"type":"fade","dur_s":0.15,"scale_from":1.0} | {"type":"fade","scale_first":null,"presence_first":0.274,"dur_s":0.067,"scales":[null,nul… | 같다 | 아니오 | t=0.33s [프레임](frames/cap_c_spk_0000570.png) |
...
| 항목 | 레퍼런스 | 기대 | 출력 측정 | 판정(같다/다르다/못 잼) | 의도한 변경 | 근거 |
|---|---|---|---|---|---|---|
| 전환 종류·길이 [s2] | 못 잼 | {"type":"cut","dur":0.0} | {"type":"cut","dur":0.0,"score":38.48} | 같다 | 아니오 | t=4.00s |
...
| 확대(줌) [s1] | 못 잼 | {"final_ratio":1.25,"t50":1.374,"dur":0.35,"ease":"out","center_canvas":[731.4,971.3]} | {"measured_final_ratio":1.2499,"source_ratio":1.0001,"zoom_ratio_corrected":1.2497,"measu… | 같다 | 아니오 | t=1.40s ORB+RANSAC 유사변환 배율(출력 프레임끼리 비교, 소스 자체의 배율 변화로 나눔) |
| 줌 없음 확인 [s2] | — | {"final_ratio":1.0} | {"max_dev":0.0011,"final_ratio":0.9994,"source_ratio":0.9994,"corrected":1.0,"median_inli… | 같다 (참고) | 아니오 | 소스 자체의 카메라 줌도 여기에 잡힘 |
...
| 줌 배율·길이 (레퍼런스 대비) | 못 잼 | {"motion.zoom.scale_to":1.25,"motion.zoom.dur_s":0.35,"motion.zoom.ease":"out"} | {"final_ratio":1.2499} | 못 잼 (참고) | 아니오 | 레퍼런스 미측정(임시값) 키 3개: motion.zoom.scale_to, motion.zoom.dur_s, motion.z… |
| 정지(프리즈) [s2] | 못 잼 | {"start":6.8,"hold":0.7} | {"presence":"present","start":6.8,"hold":0.667,"mean_diff":0.0107} | 같다 | 아니오 | t=6.80s |
| 계획에 없는 정지 화면 | — | [] | [] | 같다 | 아니오 | 소스도 정지해 있으면 제외; 판단 불가(소스 없음)면 포함 |
| 영상당 정지 횟수 | {"motion.freeze.max_per_video":"해당 없음(rule)"} | {"max":2} | {"count":1} | 같다 | 아니오 | — |
| 정지 길이 (레퍼런스 대비) | 못 잼 | {"motion.freeze.hold_s":0.7} | {"hold_s":0.667} | 못 잼 (참고) | 아니오 | 레퍼런스 미측정(임시값) 키 1개: motion.freeze.hold_s — 출력이 임시값과 같아도 레퍼런스와 같다고 판정하… |
...
| 항목 | 레퍼런스 | 기대 | 출력 측정 | 판정(같다/다르다/못 잼) | 의도한 변경 | 근거 |
|---|---|---|---|---|---|---|
| BGM 곡·버전 일치(파형 대조) | 못 잼 | {"path":"assets/test/generated/music_bed_a.wav","track_id":null} | {"presence":"present","waveform_ncc":0.7087,"local_match":{"n":84,"q95":0.9999,"q70":0.99… | 같다 | 아니오 | 출력 믹스에서 계획한 음악 파일의 파형을 찾음 |
| BGM 속도(버전) | 못 잼 | 1.0 | 1.0 | 같다 | 아니오 | 후보 템포 상위: [[1.0, 0.6454], [1.005, 0.2163], [0.995, 0.1924], [1.015, 0… |
...
| 항목 | 레퍼런스 | 기대 | 출력 측정 | 판정(같다/다르다/못 잼) | 의도한 변경 | 근거 |
|---|---|---|---|---|---|---|
| 보존 원음 [s3 10.00–12.30s] | 못 잼 | {"present":true,"gain_db":0.0,"stem":"raw","reason":"소스에 들어 있는 대사 한 줄(합성 TTS)을 살림 — 원음 보존… | {"presence":"present","present_fraction":1.0,"gain_db_mix_scale":-11.22} | 같다 | 아니오 | t=10.00s 원음 이득은 BGM 기준 믹스 척도 추정(BGM 없으면 없음) |
...
| 보존 원음 크기·경계 (레퍼런스 대비) | 못 잼 | {"audio.original.keep_gain_db":0.0,"audio.original.fade_s":0.04} | {"gain_db":-11.87} | 못 잼 (참고) | 아니오 | 레퍼런스 미측정(임시값) 키 2개: audio.original.keep_gain_db, audio.original.fade_… |

### 효과음 종류별 개수

| 항목 | 레퍼런스 | 기대 | 출력 측정 | 판정(같다/다르다/못 잼) | 의도한 변경 | 근거 |
|---|---|---|---|---|---|---|
| 효과음 [fx1·whoosh] 위치 | — | {"t":1.25,"type":"whoosh"} | {"t":1.2522,"ncc":0.852} | 같다 | 아니오 | t=1.25s |
| 효과음 [fx1·whoosh] 크기 | 못 잼 | {"gain_db":-8.0} | {"gain_db_mix_scale":-27.8,"reference":"local_bgm"} | 다르다 (참고) | 아니오 | t=1.25s |
| 효과음 [fx2·click] 위치 | — | {"t":6.8,"type":"click"} | {"t":6.8,"ncc":0.99} | 같다 | 아니오 | t=6.80s |
| 효과음 [fx2·click] 크기 | 못 잼 | {"gain_db":-8.0} | {"gain_db_mix_scale":-23.83,"reference":"bgm_plateau (BGM 없음/정적 구간)"} | 다르다 (참고) | 아니오 | t=6.80s |
| 효과음 [fx3·pop] 위치 | — | {"t":7.75,"type":"pop"} | {"t":7.75,"ncc":0.944} | 같다 | 아니오 | t=7.75s |
| 효과음 [fx3·pop] 크기 | 못 잼 | {"gain_db":-8.0} | {"gain_db_mix_scale":-20.21,"reference":"local_bgm"} | 다르다 (참고) | 아니오 | t=7.75s |
| 효과음 [fx4·ding] 위치 | — | {"t":10.9,"type":"ding"} | {"t":10.9357,"ncc":0.949} | 같다 | 아니오 | t=10.94s |
| 효과음 [fx4·ding] 크기 | 못 잼 | {"gain_db":-8.0} | {"gain_db_mix_scale":-18.8,"reference":"local_bgm"} | 다르다 (참고) | 아니오 | t=10.94s |
| 효과음 [fx5·boing] 위치 | — | {"t":15.0,"type":"boing"} | {"t":15.0,"ncc":0.866} | 같다 | 아니오 | t=15.00s |
| 효과음 [fx5·boing] 크기 | 못 잼 | {"gain_db":-8.0} | {"gain_db_mix_scale":-17.44,"reference":"local_bgm"} | 다르다 (참고) | 아니오 | t=15.00s |
| 효과음 개수 [boing] (계획 대비) | — | 1 | 1 | 같다 | 아니오 | t=15.00s |
| 효과음 개수 [click] (계획 대비) | — | 1 | 1 | 같다 | 아니오 | t=6.80s |
| 효과음 개수 [ding] (계획 대비) | — | 1 | 1 | 같다 | 아니오 | t=10.94s |
...
| 효과음 크기 (레퍼런스 대비) | 못 잼 | {"audio.sfx.gain_db_default":-8.0} | {"gain_db":-20.21} | 못 잼 (참고) | 아니오 | 레퍼런스 미측정(임시값) 키 1개: audio.sfx.gain_db_default — 출력이 임시값과 같아도 레퍼런스와 같다… |
...
- 얼굴 가림(얼굴 검출) (`cover_up.faces:detector`): 얼굴 검출기 없음: 이 OpenCV 빌드(cv2 5.0.0)에는 Haar cascade(CascadeClassifier/cv2.data)가 없고, 얼굴 모델 파일(assets/models/face_detection_yunet*.onnx 또는 $SHORTKIT_FACE_MODEL)도 없음 — plan 보호 영역 검사로 대체 — 제작 영향: 얼굴·손·핵심 물체를 가리는지 확인하지 못함
- BGM 곡·버전·속도·구간 (레퍼런스 대비) (`audio.bgm:bgm_ref`): 레퍼런스 미측정(임시값) 키 9개: audio.bgm.track_id, audio.bgm.title, audio.bgm.version, audio.bgm.tempo_ratio, audio.bgm.section_start_s, audio.bgm.gain_db … — 출력이 임시값과 같아도 레퍼런스와 같다고 판정하지 않음. 레퍼런스 곡 식별(제목·버전·속도·구간)이 필요 — 제작 영향: BGM 곡/버전/속도/구간/크기 불일치 → 음악 일치 판정 불가
...
## 결함 기록 (defects.jsonl)
- 열림 8 / 이번에 해결 확인 2 / 재발 0 / 새로 등록 0
- 같은 검사 재확인: test-qa-bad: audio.sfx.count=problem, audio.sfx.no_event=problem, audio.sfx.placement=ok, caption.font=problem, cover.frame=ok
- 같은 검사 재확인: test-qa-good: audio.sfx.count=ok, audio.sfx.no_event=ok, audio.sfx.placement=ok, caption.font=problem, cover.frame=ok
